...
|---------|------------------|
| `users` | `name`, `email` (unique), `password`, `currency` (default `EUR`), `locale` (default `it`), `date_format` (default `d/m/Y`, whitelist in `config/finance.php` → `finance.date_formats`), `notification_preferences` (JSON nullable: preferenze notifiche per-utente) |
...
| PUT | `/api/auth/preferences` | `auth:sanctum` | Aggiorna le preferenze di visualizzazione (`date_format`, validato sulla whitelist `finance.date_formats`), ritorna `UserResource` |
