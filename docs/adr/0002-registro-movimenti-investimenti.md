...
- **U2 — Rendimento annualizzato (XIRR/TWR).** *(XIRR implementato il 2026-09-25 come `xirr_pct` su `/investments/history`; TWR resta aperto.)* I dati ci sono già tutti: il registro ha i flussi con le date, la serie ha i valori. È solo un calcolo da aggiungere a `InvestmentHistoryService`. Nessuna decisione da ribaltare.
